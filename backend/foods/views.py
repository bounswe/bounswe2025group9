...
sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'api', 'db_initialization'))
...
class GetOrFetchFoodEntry(APIView):
    def get(self, request):
        ...
        try:
            ...
            if not foods:
                return Response({"error": "Food not found in FatSecret API"}, status=status.HTTP_404_NOT_FOUND)
            ...
            if not parsed:
                return Response({"error": "Could not parse FatSecret response"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
