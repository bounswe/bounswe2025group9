...
def update_food_images(foods: List[Dict[str, Any]], limit: int = None) -> List[Dict[str, Any]]:
    """
...
def main():
    ...
    without_images = [food for food in foods if not food['imageUrl'] or food['imageUrl'] == '']

    if without_images:
        # Allow limiting the number of API calls to avoid rate limit
        api_limit = int(input("Enter maximum number of foods to update (or 0 for all): "))
